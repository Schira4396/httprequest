...
func (r *Request) Upload2(url, filename, fileinput string, buf bytes.Buffer) (*Response, error) {
	return r.sendFile2(url, filename, fileinput, buf)
}
...
func (r *Request) sendFile2(url, filename, fileinput string, buf bytes.Buffer) (*Response, error) {
	if url == "" {
		return nil, errors.New("parameter url is required")
	}
	...
	fileBuffer := &bytes.Buffer{}
	bodyWriter := multipart.NewWriter(fileBuffer)
	fileWriter, er := bodyWriter.CreateFormFile(fileinput, "qq")
	if er != nil {
		return nil, er
	}

	f, er := os.Open("main.go")
	if er != nil {
		return nil, er
	}
	defer f.Close()

	// _, er = io.Copy(fileWriter, io.MultiReader(&buf))
	// if er != nil {
	// 	return nil, er
	// }
	a := bytes.NewReader(buf.Bytes())
	// a, err := buf.WriteTo(&fileBuffer)
	a.WriteTo(fileWriter)
	// fmt.Println("qq" + buf.String())
	contentType := bodyWriter.FormDataContentType()
	bodyWriter.Close()
